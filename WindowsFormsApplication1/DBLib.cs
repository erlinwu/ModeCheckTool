...
namespace WxDbService.BaseCls
{
    ...
    {
        //数据库类型
        public enum DataBaseType { Oracle = 0, SqlServer, Access,Excel,DB2,IBMDB2DLL, none };
        //数据类型
        ...
        public int ExcutePro(string s_proname, object[] o_params) //执行存储过程
        {
            ...
            else if (dbtype == DataBaseType.IBMDB2DLL)
            {
                try
                {
                    ...
                    foreach (object o_param in o_params)
                    {
                        myDB2Command.Parameters.Add((SqlParameter)o_param);
                    }
                    i_result = myDB2Command.ExecuteNonQuery();
        ...
        /// 大批量数据执行
        /// </summary>
        /// <param name="s_sqls"></param>
        /// <param name="limit"></param>
        /// <returns></returns>
        //public string HugeSqlsCommit(string[] s_sqls, int limit_count)
        //{
        //    try
        //    {
        //        int count = 0;
        ...
        //        int last = s_sqls.Length;


        //        while (last > 0)
        //        {
        //            List<string> l_sqls = new List<string>();
        //            if (last > limit)
        //            {
        //                for (int i = 0; i < limit; i++)
        //                {
        //                    l_sqls.Add(s_sqls[count + i]);
        //                }
        //            }
        //            else
        //            {
        ...
        //            if (l_sqls.Count > 0)
        //            {
        //                int i_result = this.ExcuteSqls(l_sqls.ToArray());
        //                if (i_result > 0)
        //                {
        //                    succeed = succeed + l_sqls.Count;
        //                }
        //                else
        //                {
        //                    fail = fail + l_sqls.Count;
        //                }
        //            }
        //            else
        //            {
        //                return "0";
        //            }
        //            count = count + l_sqls.Count;
        //            last = last - l_sqls.Count;
        //        }
        //        return count.ToString() + "#" + succeed.ToString() + "#" + fail.ToString();
        //    }
        //    catch
        //    {
        //        return "-1";
        ...
        /// <summary>
        /// 获取数据集扩展 可以指定返回数量 返回DataClass对象集合
        /// </summary>
        /// <param name="s_sql"></param>
        /// <param name="rowcount"></param>
        /// <returns></returns>
        public DataClass[] GetDataEx(string s_sql, int rowcount)
        {
            try
            {
                if (rowcount > 0)
                {
                    ...
                    {
                        s_sql = "select * from (" + s_sql + ") where rownum<" + (rowcount + 1).ToString();
                    }
                    else if (this.DBType == DataBaseType.SqlServer)
                    {
                        s_sql = s_sql.Replace("select ", "select Top " + rowcount.ToString() + " ");
                    }
                    else if (this.DBType == DataBaseType.IBMDB2DLL )
                    {
                        s_sql = s_sql + " fetch first " + rowcount.ToString() + " rows only";
                    }
                    else
                    {
                ...
                DataSet ds = this.GetData(s_sql);
                List<DataClass> lst_dc = new List<DataClass>();
                int rcount = 0;
                int ccount = DataClass.C_PROPNUM;
                if (rowcount == 0)
                {
                ...
                {
                    rcount = (rowcount > ds.Tables[0].Rows.Count) ? ds.Tables[0].Rows.Count : rowcount;
                }

                if (ds.Tables[0].Columns.Count < ccount)
                {
                ...
                {
                    DataClass tmp_headerdc = new DataClass();
                    for (int i = 0; i < ccount; i++)
                    {
                        tmp_headerdc.keyvalue[i] = ds.Tables[0].Columns[i].ColumnName;
                    }
                    lst_dc.Add(tmp_headerdc);
                    ...
                    for (int i = 0; i < rcount; i++)
                    {
                        ...
                        {
                            dc.keyvalue[j] = ds.Tables[0].Rows[i][j].ToString();
                        }
                        lst_dc.Add(dc);
                    }
                    return lst_dc.ToArray();
                }
                else
                {
                    return null;
